...
void Application::Renderer::Initialize(Application::Window const& window, std::int32_t index, std::uint32_t flags) {
    Application::IDependency<SDL_Renderer, decltype(SDL_CreateRenderer), decltype(SDL_DestroyRenderer)>::Initialize<SDL_CreateRenderer, SDL_DestroyRenderer>(window.Get(), index, flags);
}
